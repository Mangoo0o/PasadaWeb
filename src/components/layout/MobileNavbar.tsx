...
export const MobileNavbar: React.FC<MobileNavbarProps> = ({ activeTab, setActiveTab }) => {
  ...
  return (
    <div className="fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-50 pointer-events-auto animate-in fade-in slide-in-from-bottom-5 duration-300">
      <nav 
        className="relative bg-[#0052d1]/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-[36px] sm:rounded-[44px] p-1.5 flex items-center shadow-[0_12px_40px_rgba(0,82,209,0.42)] border border-white/30"
        role="tablist"
        ...
        {tabs.map((tab, idx) => {
          ...
          return (
            <button
              ...
              title={tab.label}
              className="relative z-10 h-11 sm:h-12 flex-1 min-w-[72px] sm:min-w-[100px] px-2.5 sm:px-5 flex items-center justify-center gap-1.5 sm:gap-2 rounded-full select-none cursor-pointer active:scale-95 transition-all duration-200"
            >
              <Icon 
                className={`w-4 h-4 sm:w-5 sm:h-5 transition-all duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] shrink-0 ${
                  isActive 
                    ? 'text-[#0052d1] scale-110 -translate-y-0.5' 
                    : 'text-white/80 hover:text-white scale-100'
                }`} 
              />
              <span 
                className={`text-xs sm:text-sm font-extrabold transition-colors duration-200 whitespace-nowrap ${
                  isActive 
                    ? 'text-[#0052d1] font-black' 
