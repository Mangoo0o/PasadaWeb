  ...
  return (
    <div className="fixed bottom-4 sm:bottom-6 left-1/2 -translate-x-1/2 z-50 pointer-events-auto animate-in fade-in slide-in-from-bottom-5 duration-300 select-none">
      ...
      >
        {tabs.map((tab) => {
          const Icon = tab.icon;
          const isActive = tab.id === activeTab || (tab.id === 'profile' && activeTab === 'history' && user?.role === 'passenger');
            ...
            >
              <Icon 
                className={`w-4 h-4 sm:w-5 sm:h-5 transition-transform duration-300 ease-[cubic-bezier(0.34,1.56,0.64,1)] shrink-0 ${
                  isActive ? 'text-[#0052d1] scale-110' : 'text-white scale-100'
                }`} 
              />
              {isActive && (
                <span className="text-xs sm:text-sm font-black text-[#0052d1] whitespace-nowrap animate-in fade-in zoom-in-95 duration-200">
                  {tab.label}
                </span>
              )}
            </button>
